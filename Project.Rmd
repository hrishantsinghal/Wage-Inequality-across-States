...
The study aims to understand the factors that are responsible for the variation in casual labor wage disparities across Indian States. 
...
#### 1. **[India Wage Report, 2018](https://www.ilo.org/wcmsp5/groups/public/---asia/---ro-bangkok/---sro-new_delhi/documents/publication/wcms_638305.pdf) – Considered the association between the SDP of each state and its nominal casual labor wages and found a very weak correlation of 0.5 so it concluded that no conclusion could be drawn from that. **

ILO looked separately at the rural and urban wage correlation with the nominal casual    labor wages, which implied that it believed there is little association between rural      and urban wages of a state.

I find that there is a strong association between urban and rural wages. 
...
So, we can look at the per capita SDP of the state and its average wages (without making distinctions for urban and rural areas), I find a correlation value of 0.73. 
...
#### **2.	[AV Jose, 2013](https://www.jstor.org/stable/23527238) – Land and Labor Productivity can explain differences in Rural Wages across Indian States**
...
AV Jose was right that land and labour productivity does affect farmer’s income but since there are some states like Jharkhand, Uttarakhand, Rajasthan etc where proportion of farmers in the casual labour of rural areas is too low so land and labour productivity does not have any effect on rural income in general. 
...
Casual Labour is paid the most poorly among all the kinds of jobs. If people engaged in casual labor are higher in a state, then it shows that the state is not well developed, people are not well educated and need to do informal jobs so we would expect the average wages to be low for such a state. 
...
$Wage_i$=$B_1$+$B_2$*$(Per Capita SDP)_i$+$B_3$*$(Percentage of People Engaged in Casual Labor)_i$+$e_i$
...
stargazer(regression_model,type="text",align=TRUE,title="Regression Results")
```

## Conclusion 
We find that there is a positive association with wages of casual labour in per capita SDP of Indian States. When the state grows richer, it increases the wages of the least well of along with it. 
...
## Variables that had to be left out of the model due to issues of multicolinearity or insignificant results . 

Literate People have higher income on average.
...
cat(paste("But the correlation Coefficient between wages and percentage of literacy in each state is ",round(cor(pd_dat$n_prop,pd_dat$wage),2),", which is very insignificant.",sep=""))
...
But due to issues of multicollinearity and insignificant results, these variables had to be left out of the model. 

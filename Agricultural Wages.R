...
income_data<-main_data %>% 
  ...
  drop_na() %>% filter(WS5=="(00) Agriculture 0") %>% 
...
pd_dat %>% as_survey(weights = c(weight)) %>% 
  group_by(ED6) %>% 
  summarise(wage=round(survey_mean(total_wage_by_total_work),2))
...
pd_dat %>% as_survey(weights = c(weight)) %>% 
  group_by(state,URBAN2011) %>% summarise(n=survey_total()) %>% 
  mutate(prp_in_agri=n*100/sum(n)) %>% filter(URBAN2011=="(1) urban 1") %>% 
  filter(prp_in_agri>=10)
...
regression_model<-lm(wage~psu+agri_wage,wage_gdp_urban)
